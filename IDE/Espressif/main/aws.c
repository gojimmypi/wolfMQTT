...
static int mqtt_aws_tls_cb(MqttClient* client)
{
    ...
    if (client->tls.ctx) {
        ...
        rc = wolfSSL_CTX_load_verify_buffer(client->tls.ctx,
            (const byte*)root_ca,
            (long)XSTRLEN(root_ca),
            WOLFSSL_FILETYPE_PEM);

        /* Load Client Cert */
        if (rc == WOLFSSL_SUCCESS)
            rc = wolfSSL_CTX_use_certificate_buffer(client->tls.ctx,
                (const byte*)device_cert,
                (long)XSTRLEN(device_cert),
                WOLFSSL_FILETYPE_PEM);

        /* Load Private Key */
        if (rc == WOLFSSL_SUCCESS)
            rc = wolfSSL_CTX_use_PrivateKey_buffer(client->tls.ctx,
                (const byte*)device_priv_key,
                (long)XSTRLEN(device_priv_key),
                WOLFSSL_FILETYPE_PEM);
    }
...
static int mqtt_message_cb(MqttClient *client,
    MqttMessage *msg,
    byte msg_new,
    byte msg_done)
{
    ...
    case WMQ_INIT:
        {
            ...
            rc = MqttClient_Init(&mqttCtx->client,
                &mqttCtx->net,
                mqtt_message_cb,
                mqttCtx->tx_buf,
                MAX_BUFFER_SIZE,
    ...
    case WMQ_TCP_CONN:
        {
            ...
            /* Connect to broker */
            rc = MqttClient_NetConnect(&mqttCtx->client,
                mqttCtx->host,
                mqttCtx->port,
    ...
    case WMQ_SUB:
        {
            ...
            /* Publish Topic */
            XSNPRINTF((char*)mqttCtx->app_ctx,
                AWSIOT_PUBLISH_MSG_SZ,
...
int main_aws()
{
    ...
    /* parse arguments */
    // TODO rc = mqtt_parse_args(&mqttCtx, argc, argv);
    rc = 0;
    if (rc != 0) {
        return rc;
    }
